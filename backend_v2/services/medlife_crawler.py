...
class MedLifeCrawler(BaseCrawler):
    """
    Crawler for MedLife Romania patient portal.

    MedLife uses a patient portal for accessing lab results and medical documents.
    The portal may require email/CNP login and possibly CAPTCHA verification.
    """

    def __init__(self, headless: bool = True, user_id: int = None):
        super().__init__("medlife", headless, user_id)
        # MedLife portal URLs - these may need verification
        self.login_url = "https://portal.medlife.ro/auth/login"
        self.dashboard_url = "https://portal.medlife.ro/dashboard"
        self.results_url = "https://portal.medlife.ro/rezultate"
        # Alternative URLs to try if primary doesn't work
        self.alt_login_urls = [
            "https://contulmeu.medlife.ro/login",
            "https://rezultate.medlife.ro/login",
            "https://www.medlife.ro/cont",
        ]
    ...
    def _check_authenticated(self, page: Page) -> bool:
        ...
        try:
            ...
            # Only check URL for login pages
            ...
            if any(x in url for x in ["dashboard", "rezultate", "cont", "profil"]) and "login" not in url:
                return True
    ...
    def _find_and_try_login_url(self, page: Page) -> str:
        ...
        try:
            page.goto("https://www.medlife.ro", wait_until="domcontentloaded")
            self._dismiss_cookie_consent(page)

            login_link = page.locator("a:has-text('Cont'), a:has-text('Login'), a:has-text('Autentificare')").first
    ...
    def login_sync(self, page: Page, credentials: Dict[str, str]):
        """Perform login to MedLife portal."""
        ...
        for i, elem in enumerate(download_elements[:50]):  # Limit to 50
            try:
                ...
                try:
                    ...
                    if href:
                        try:
                            download_success = self._download_via_http(href, target_path, cookie_header)
    ...
    def _download_via_http(self, url: str, target_path: str, cookie_header: str) -> bool:
                ...
                for dl_selector in ["a[href*='.pdf']", "button:has-text('Descarca')", "a:has-text('Descarca')"]:
                    try:
                        ...
                        if dl_btn.count() > 0 and dl_btn.is_visible():
                            filename = f"medlife_detail_{i}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
                            target_path = os.path.join(self.download_dir, filename)
